...
;(
	function( $, window, document, undefined ) {
		...
		$.parallaxBackground = function( element, options ) {
			...
			var $parallax 	= null;
			var $window 	= $(window);
			var $parallax_inner, d, e, h, p, s, t, w;
			...
			plugin.setup = function() {
				...
				d = plugin.resizeBackground($parallax);
			...
			plugin.resizeBackground = function( el ) {
				w = el.outerWidth();
				h = el.outerHeight();
				...
				if (plugin.opts.parallaxDirection === 'left' || plugin.opts.parallaxDirection === 'right') {
					w += f * (ww * Math.abs(parseFloat(plugin.opts.parallaxSpeed)));
				}

				if (plugin.opts.parallaxDirection === 'up' || plugin.opts.parallaxDirection === 'down') {
					h += f * (wh * Math.abs(parseFloat(plugin.opts.parallaxSpeed)));
				}
