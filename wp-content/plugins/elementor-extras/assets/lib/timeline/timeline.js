		...
		$.timeline = function(element, options) {
			...
			plugin.setup = function() {
				...
				$line.css({
					...
					'bottom'	: ( $element.offset().top + $element.outerHeight() ) - $cards.last().find('.point').offset().top
				});
			...
			plugin.progress = function() {
				...
				var _last_pos = $cards.last().find('.point').offset().top,
				...
				$cards.each( function() {
					if ( $(this).find('.point').offset().top < ( $window.scrollTop() + $viewport.outerHeight() / 2 ) ) {
