    ...
    function( $, window, document, undefined ) {
		...
		$.hotips = function(element, options) {
			...
			plugin._construct = function() {
				...
				if ( ! $content_element.length || $content_element.is(':empty') )
					return false;
			...
			plugin.update = function() {
				...
				if ( $window.width() < $target.outerWidth() * 1.5 )
			...
			plugin.hide = function() {
				...
				if ( plugin.opts.position === 'top' ) {
					...
					TweenMax.to( $tooltip, 0.2, { top : _tTop + _offset, autoAlpha : 0, delay : plugin.opts.delayOut,
						onComplete : function() {
							$tooltip.remove();
							is_open = false;
						}
					...
					TweenMax.to( $tooltip, 0.2, { left : _tLeft + _offset, autoAlpha : 0, delay : plugin.opts.delayOut,
						onComplete : function() {
							$tooltip.remove();
							is_open = false;
						}
