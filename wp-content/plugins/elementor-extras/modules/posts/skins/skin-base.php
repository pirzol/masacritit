...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	public function register_layout_content_controls() {
		...
		$this->add_responsive_control(
			...
			[
				'label' 			=> __( 'Columns Spacing', 'elementor-pro' ),
		...
		$this->add_control(
			'grid_rows_spacing',
			[
				'label' 			=> __( 'Rows Spacing', 'elementor-pro' ),
				'type' 				=> Controls_Manager::SLIDER,
				'size_units' 		=> [ 'px' ],
				'default'			=> [ 'size' => 24, ],
				'tablet_default'	=> [ 'size' => 12, ],
	...
	protected function render_post_media() {
		...
		?><a <?php echo $this->parent->get_render_attribute_string( 'post-media-' . get_the_ID() ); ?>><?php
	...
	protected function render_post_media_thumbnail() {
		...
		$this->parent->add_render_attribute( 'post-thumbnail' . get_the_ID(), 'class', 'ee-post__media__thumbnail' );
	...
	protected function render_post_media_overlay() {
		?><div class="ee-post__media__overlay"></div><?php
	}
	...
	protected function render_post_terms( $area = 'header' ) {
		...
		<ul <?php echo $this->parent->get_render_attribute_string( 'post-terms-'. get_the_ID() ); ?>>
			...
			<?php foreach( $terms as $term ) {
				...
				$this->parent->add_render_attribute( $term_render_key, 'class', 'ee-post__terms__item' );

				if ( 'media' !== $area ) {
					$this->parent->add_render_attribute( $term_render_key, 'href', get_term_link( $term ) );
				}
			?>

				<li class="ee-post__terms__term">
					<<?php echo $tag; ?> <?php echo $this->parent->get_render_attribute_string( $term_render_key ); ?>>
						<?php echo $term->name; ?>
					</<?php echo $tag; ?>><?php echo $this->render_terms_separator(); ?>
				</li>
	...
	protected function render_post_title( $area = 'body' ) {
		...
		if ( $this->parent->is_in_area( 'post_title_position', 'media' ) ) {
			$title_tag = 'div';
		} else {
