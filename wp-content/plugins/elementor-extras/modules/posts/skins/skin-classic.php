...
class Skin_Classic extends Skin_Base {
	...
	public function register_layout_content_controls() {
		...
		// $this->add_control(
		// 	'masonry_layout_variable_description',
		// 	[
		// 		'type' 				=> Controls_Manager::RAW_HTML,
		// 		'raw' 				=> __( 'With Variable layout selected, make sure you change the position of your post content parts inside Media.', 'elementor-extras' ),
		// 		'content_classes' 	=> 'ee-raw-html ee-raw-html__warning',
		// 		'condition'			=> [
		// 			$this->get_control_id( 'layout' )			=> 'masonry',
		// 			$this->get_control_id( 'masonry_layout' ) 	=> 'variable',
		// 		]
		// 	]
		// );
		...
		// 		'options' 		=> [
		// 			'default' 	=> __( 'Default', 'elementor-extras' ),
		// 			'variable' 	=> __( 'Variable', 'elementor-extras' ),
		// 		],
		// 		'frontend_available' => true,
		// 		'condition'		=> [
		// 			$this->get_control_id( 'layout' )	=> 'masonry',
	...
	protected function render_post_start() {
		...
		$this->parent->add_render_attribute( 'grid-item' . get_the_ID(), [
			'class'	=> [
				...
				'ee-u-1/' . $settings[ 'columns_mobile' ] . '@tablet',
		...
		if ( '1' === $settings['columns'] && in_array( $settings[ 'post_media_position' ], array( 'left', 'right' ) ) ) {
	...
	protected function render_filters() {
		...
		$this->parent->add_render_attribute( 'filter', 'class', [
			'ee-filters__item',
			'o-nav__item',
		] );
		...
		?><ul <?php echo $this->parent->get_render_attribute_string( 'filters' ); ?>>
			<li <?php echo $this->parent->get_render_attribute_string( 'filter' ); ?>><a <?php echo $this->parent->get_render_attribute_string( $this->get_control_id( 'filters_all_text' ) ); ?>>
				<?php echo $this->get_instance_value( 'filters_all_text' ); ?>
			</a></li>
			<?php foreach ( $filters as $filter ) {

				$filter_trigger_key = 'filter-trigger-' . $filter->term_id;

				$this->parent->add_render_attribute( $filter_trigger_key, 'data-filter', '.ee-filter-' . $filter->term_id );

				?>
				<li <?php echo $this->parent->get_render_attribute_string( 'filter' ); ?>>
					<a <?php echo $this->parent->get_render_attribute_string( $filter_trigger_key ); ?>><?php echo $filter->name; ?></a>
				</li>
			<?php } ?>
		</ul><?php
	}
