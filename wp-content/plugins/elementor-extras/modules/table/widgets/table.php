	...
	protected function _register_cell_controls( $repeater, $condition = array() ) {
		...
		$repeater->add_control(
			...
			[
				...
				'placeholder' 	=> __( '', 'elementor-extras' ),
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'return_value' 	=> 'yes',
					'default'		=> 'yes',
					'description'   => __( 'Try to automatically fetch corresponding headers on mobile. Works only when column span values are not used.', 'elementor-extras' ),
					'frontend_available' => true,
					'condition'		=> [
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'label_block' 	=> false,
					'selectors' 	=> [
						'{{WRAPPER}} .ee-table__row,
						 {{WRAPPER}} .ee-table__cell' => 'transition-duration: {{VALUE}}s;',
					],
				]
			);
	...
	protected function _header_template() {
		...
		<thead>
			...
			<# _.each( settings.header_cells, function( row ) { #>
				...
				<th class="ee-table__cell elementor-repeater-item-{{ row._id }}" rowspan="{{ row.cell_row_span }}" colspan="{{ row.cell_span }}"">
